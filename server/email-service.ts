...
export class MockEmailService implements IEmailService {
  ...
  async sendNewsletterToSubscriber(newsletter: Newsletter, subscriber: Subscriber, trackingId?: number): Promise<boolean> {
    const name = subscriber.name || "there";
    const unsubscribeUrl = `${process.env.SITE_URL || 'http://localhost:3000'}/api/newsletter/unsubscribe?token=${subscriber.unsubscribeToken}`;
...
function createEmailService(): IEmailService {
  ...
  // Can extend this to support real email providers like SendGrid, etc.
  // based on EMAIL_SERVICE environment variable
