...
import { getPosts, categoryLabels, type BlogPost, type BlogCategory } from "@/lib/posts";
import SEO from "@/components/seo";
import SchemaMarkup from "@/components/schema-markup";
import { ArrowRight, BookOpen, Linkedin, Newspaper, Clock, Calendar } from "lucide-react";
...
const allCategories: Array<{ key: BlogCategory | 'all'; label: string }> = [
  { key: 'all', label: 'All' },
  { key: 'ai-agents', label: 'AI Agents' },
  { key: 'building-in-public', label: 'Building in Public' },
  { key: 'industry-takes', label: 'Industry Takes' },
...
export default function Writing() {
  ...
  const getPlatformIcon = (platform: string) => {
    switch (platform) {
      case "Medium": return <MediumIcon className="h-5 w-5" />;
  ...
  return (
    <div className="bg-[#FAF9F6] min-h-screen">
      <SEO
        ...
        type="website"
        keywords="AI insights, technology analysis, software development, machine learning"
      />
      ...
      <section className="py-16 md:py-20">
        <div className="container max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="font-serif text-2xl md:text-3xl font-semibold text-[#1A1A1A] mb-4">Where I Publish</h2>
            <p className="text-[#5C5C5C] max-w-xl mx-auto">Follow along on the platforms where I share insights regularly.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            {publications.map((pub) => (
              <a key={pub.id} href={pub.url} target="_blank" rel="noopener noreferrer" className="group">
                <article className="relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-[#E8E4DF] h-full flex flex-col">
                  <div className="relative h-48 md:h-56 overflow-hidden" style={{ background: `linear-gradient(135deg, ${pub.accentColor}15 0%, ${pub.accentColor}05 100%)` }}>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-6xl md:text-7xl opacity-20 group-hover:opacity-30 group-hover:scale-110 transition-all duration-500" style={{ color: pub.accentColor }}>
                        {getPlatformIcon(pub.platform)}
                      </div>
                    </div>
                    <div className="absolute top-4 right-4 w-24 h-24 rounded-full opacity-10" style={{ backgroundColor: pub.accentColor }} />
                    <div className="absolute bottom-4 left-4 w-16 h-16 rounded-full opacity-5" style={{ backgroundColor: pub.accentColor }} />
                    <div className="absolute top-4 left-4">
                      <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium text-white" style={{ backgroundColor: pub.accentColor }}>
                        {getPlatformIcon(pub.platform)}
                        {pub.platform}
                      </span>
                    </div>
                  ...
                  <div className="p-6 md:p-8 flex-1 flex flex-col">
                    <h3 className="font-serif text-xl md:text-2xl font-semibold text-[#1A1A1A] mb-3 group-hover:text-[#C45B3E] transition-colors duration-300">{pub.title}</h3>
                    <p className="text-[#5C5C5C] leading-relaxed mb-6 flex-1">{pub.synopsis}</p>
                    <div className="flex flex-wrap gap-2 mb-6">
                      {pub.topics.map((topic) => (
                        <span key={topic} className="px-3 py-1 text-xs font-medium rounded-full bg-[#FAF9F6] text-[#5C5C5C] border border-[#E8E4DF] group-hover:border-[#C45B3E]/30 group-hover:text-[#C45B3E] transition-colors duration-300">{topic}</span>
                      ))}
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="inline-flex items-center gap-2 font-medium group-hover:gap-3 transition-all duration-300" style={{ color: pub.accentColor }}>
                        {pub.platform === "LinkedIn" ? "Subscribe on LinkedIn" : "Read on Medium"}
                        <ArrowRight className="h-4 w-4" />
                      </span>
                    </div>
                  </div>
                  <div className="h-1 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" style={{ backgroundColor: pub.accentColor }} />
                </article>
              </a>
            ))}
          </div>
      ...
      <section className="pb-20 md:pb-28">
        <div className="container max-w-2xl mx-auto px-4 sm:px-6 text-center">
          <p className="text-[#5C5C5C] italic">New essays and insights published regularly. Follow on your preferred platform to stay updated.</p>
        </div>
      </section>
    </div>
  );
